...
import models.t_payment_jingtum;
...
public class Bill implements Serializable{
	private static final long serialVersionUID = -2203137347576348229L;
	public long id;
    private long _id;
	...
	/**
 	...
 	public int addInvestBills(long bidId, long repaymentId, long userId , boolean isSecBid, ErrorInfo error){
		...
		//纠偏利息
		  double interest_times=Constants.INTEREST_TIMES;
		  ...
		  if(interest_times>0){
			  ...
			  Double correct_interest = 0d;
			  Double repayment_interest = 0d;
			  List<Map<String, Object>> investList = new ArrayList<Map<String,Object>>();
			  try {
				  correct_interest = t_invests.find("select sum(correct_interest) from t_invests where bid_id= ?", bidId).first();
				  repayment_interest = t_bills.find("select sum(repayment_interest) from t_bills where bid_id = ?", bidId).first();
				  investList=t_invests.find("select new Map(id as id,amount as amount,correct_interest as correct_interest) from t_invests where bid_id= ? ", bidId).fetch();
			  } catch (Exception e) {
				  e.printStackTrace();
				  Logger.info("纠偏数据初始化时："+e.getMessage());
				  error.code = -3;
			  ...
			  if(correct_interest < repayment_interest){
				  
				  double check_amount=repayment_interest-correct_interest;
				  
				  for (Map<String, Object> map : investList) {
					  JSONObject json = JSONObject.fromObject(map);
					  long investId = json.getLong("id");//每个投资记录id
					  double amount=json.getDouble("amount");//投资金额
					  double receiveInterest = json.getDouble("correct_interest");//投资利息
					  
					  double check_interest=amount*repayment_interest/bid.amount;
				      ...
				      if(Double.parseDouble(formater.format(check_interest*interest_times))-receiveInterest<=check_amount){
						  
						  check_amount=check_amount+receiveInterest-Double.parseDouble(formater.format(check_interest*interest_times));
						  receiveInterest=Double.parseDouble(formater.format(check_interest*interest_times));
					  }
					  else{
						  
						  receiveInterest=receiveInterest+check_amount;
						  check_amount=0d;
					  }
					  
					  String updateSql = "update t_invests set correct_interest = ? where id = ?";
				 		
				      Query queryInvests = em.createQuery(updateSql).setParameter(1, receiveInterest).setParameter(2, investId);
				      
				      try {
				    	  rows = queryInvests.executeUpdate();
				      } catch(Exception e) {
				    	  e.printStackTrace();
				    	  Logger.info("纠偏数据初始化时："+e.getMessage());
				    	  error.code = -6;
		  ...
		  try {
				...
				e.printStackTrace();
				Logger.info("投资应收款明细资金时："+e.getMessage());
				error.code = -3;
...
//				Cache.set("pepaymentCache_"+paymentCacheNo, mapList, Constants.CACHE_TIME);
 	...
 	public int normalPayment(long bidId, long userId, double repaymentCorpus, double repaymentInterest, double balance, 
		...
		t_bid_jingtum jing=new t_bid_jingtum();
		try {
			jing=jing.find("bid_id",bidId).first();//获取标的地址  作为转账货币
		} catch (Exception e) {
			e.printStackTrace();
			error.code = -1;
			error.msg = "查询 标的信息错误";
		...
		users=users.findById(userId);//借款人信息
		if(users == null){
			error.code = -1;
			error.msg = "查询 用户信息错误";
			return error.code;
		...
		//获取标的持有人列表
		Jingtum japi=new Jingtum();
		List<Map<String, String>> listMap=new ArrayList<Map<String,String>>();
		try {
			listMap = japi.getJingTumList(jing.bid_address);
		} catch (JingtumCallException e1) {
...
//				double totalBalance = Arith.sub(Arith.add(investerBalance, receive), manageFee);//计算用户的可用余额
				...
				t_users useid=t_users.findById(investUserId);
				if(useid == null){
					error.code = -1;
					error.msg = "查询投资人失败";
					return error.code;
				...
				for (int i = 0; i < listMap.size(); i++) {
					if(useid.wallet_address.equals(listMap.get(i).get("counterparty")) && listMap.get(i).get("value").startsWith("-")){//根据银关的标的持有人的列表转账
						t_payment_jingtum payinfo=new t_payment_jingtum();
						try {
							payinfo.bid_id=bidId;
							payinfo.bid_address=jing.bid_address;
							payinfo.bid_user_id=bidId;
							payinfo.bid_pay_id=investUserId;
							payinfo.create_time=new Date();
							payinfo.invest_amount=receive+"";
							payinfo.tong_amount=receiveCorpus+"";
							payinfo.periods=period;
							payinfo.result=false;
							payinfo.tong_result=false;
							String [] string=japi.transferToBorrowers(useid.wallet_address, useid.wallet_secret, receiveCorpus+"",Constants.P2X_ADDRESS, jing.bid_address, jing.bid_address);//收回标的 融资通
							if("tesSUCCESS".equals(string[3])){
								payinfo.tong_result=true;
							}
							
							String [] strings=japi.transferToBorrowers(users.wallet_address, users.wallet_secret, receive+"", useid.wallet_address, Constants.CURRENCY_CNY, Constants.CURRENCY_CNY);//向用户还款
							if("tesSUCCESS".equals(strings[3])){
								result=true;
								payinfo.result=true;
							}
						} catch (Exception e) {
		...
		Query userBalance = em.createQuery(userBalanceSql).setParameter(1, payment).setParameter(2, userId);
 	...
 	public int overduePayment(long bidId, long userId, double repaymentCorpus, double repaymentInterest, double balance, 
		...
		/**
		 * 先查出来系统内的标的持有人列表
		 * 再查出来井通平台所记录的成功的投资记录，和最终持有的人的列表【如果标 被转让 或者 买卖】
		 * 发现匹配后进行转账，然后去除本条记录，提高效率【要保证，所以投资记录，转账记录，是成功的，或被修改为成功】
		 */
		t_bid_jingtum jing=new t_bid_jingtum();
		try {
			jing=jing.find("bid_id",bidId).first();//获取标的地址  作为转账货币
		...
		List<Map<String, String>> listMap=new ArrayList<Map<String,String>>();
		try {
			listMap = japi.getJingTumList(jing.bid_address);//获取标的持有人列表
		} catch (JingtumCallException e1) {
			e1.printStackTrace();
		}
		//5.非本金垫付还款的操作
		if (status == Constants.NO_REPAYMENT || true){
...
//		  		"t3.user_id = t6.user_id and t3.invest_id = t6.id) t2 set t1.overdue_fine = check_fine, t1.status = -2 where t1.id = t2.id";
		  ...
		  if(Constants.INTEREST_TIMES>0){
			  String updateInvestBillSql = "update t_bill_invests t1,(select t3.id, case when round(((t4.repayment_interest*t6.amount)/t4.repayment_corpus * ?),2)-t6.correct_interest>round((t4.overdue_fine * ifnull(t6.amount,0) / t5.amount), 2) " +
				  		" then round((t4.overdue_fine * ifnull(t6.amount,0) / t5.amount), 2) " +
				  		"else round(((t4.repayment_interest*t6.amount)/t4.repayment_corpus * ?),2)-t6.correct_interest end as check_fine, t3.periods, t3.bid_id from " +
				  		"(select b.bid_id, b.periods, b.overdue_fine,b.repayment_corpus,b.repayment_interest from t_bills b where b.overdue_mark in(-1,-2,-3) and b.status in(-1,-2)) t4 " +
				  		"left join (select a.invest_id, a.id, a.periods, a.bid_id,a.user_id from t_bill_invests a where a.status in(-1, -2)) t3 on t3.bid_id = t4.bid_id and t3.periods = t4.periods " +
				  		"left join (select c.id, c.amount from t_bids c) t5 on t3.bid_id = t5.id " +
				  		"left join (select d.id, d.bid_id, d.user_id,d.amount,d.correct_interest from t_invests d) t6 on t3.bid_id = t6.bid_id and t3.user_id = t6.user_id and t3.invest_id = t6.id) t2 " +
				  		"set t1.overdue_fine = check_fine, t1.status = -2 where t1.id = t2.id";
					  Query updateInvestBill = em.createNativeQuery(updateInvestBillSql).setParameter(1, Constants.INTEREST_TIMES).setParameter(2, Constants.INTEREST_TIMES);
			  ...
			  try {
				  billList=t_bills.find("select new Map(id as id,bid_id as bid_id,periods as periods,repayment_corpus as repayment_corpus,repayment_interest as repayment_interest,overdue_fine as overdue_fine) from t_bills where overdue_mark= -1 ").fetch();
			  } catch (Exception e) {
				  e.printStackTrace();
				  Logger.info("修改理财账单逾期生成的数据时："+e.getMessage());
				  error.code = -3;
			  ...
			  for (Map<String, Object> map : billList) {
				  JSONObject json = JSONObject.fromObject(map);
				  long id = json.getLong("id");//每个账单记录id
				  long bid_id = json.getLong("bid_id");//标的id
				  int periods=json.getInt("periods");//账单期数
				  double repayment_corpus = json.getDouble("repayment_corpus");//账单本金
				  double repayment_interest = json.getDouble("repayment_interest");//账单利息
				  double overdue_fine = json.getDouble("overdue_fine");//账单逾期罚款
				  
				  Bid bid=new Bid();
				  bid.id=bid_id;
				  
				  double monthRate = Double.valueOf(bid.apr * 0.01)/12.0;//通过年利率得到月利率
				  
				  if(bid.periodUnit==Constants.DAY){
					  repayment_interest=Arith.mul(Arith.mul(repayment_corpus, monthRate), bid.period)/30;
				  }else{
					  ...
					  //等额本息还款（否则一次还款或先息后本）
					  if(bid.repayment.id == Constants.PAID_MONTH_EQUAL_PRINCIPAL_INTEREST){
						  double monPay = Double.valueOf(Arith.mul(repayment_corpus, monthRate) * Math.pow((1 + monthRate), period))/ 
						  Double.valueOf(Math.pow((1 + monthRate), period) - 1);//每个月要还的本金和利息
						  repayment_interest = Arith.sub(Arith.mul(monPay, period), repayment_corpus);
							
					  }else{
						  repayment_interest = Arith.round(Arith.mul(Arith.mul(repayment_corpus, monthRate), period), 2);
					  }
				  }
				  
				  Double sum_overdue_fine=0d;
				  List<Map<String, Object>> billInvestList = new ArrayList<Map<String,Object>>();
				  try {
					  sum_overdue_fine=t_bill_invests.find("select sum(overdue_fine) from t_bill_invests where bid_id= ? and periods =?",bid_id,periods).first();
					  billInvestList=t_bill_invests.find("select new Map(id as id,receive_corpus as receive_corpus,receive_interest as receive_interest,overdue_fine as overdue_fine) from t_bill_invests where bid_id= ? and periods= ? ",bid_id,periods).fetch();
				  } catch (Exception e) {
					  e.printStackTrace();
					  Logger.info("修改理财账单逾期生成的数据时："+e.getMessage());
					  error.code = -3;
				  ...
				  for (Map<String, Object> billInvestMap : billInvestList) {
					  JSONObject billInvestJson = JSONObject.fromObject(billInvestMap);
					  long billInvestId = billInvestJson.getLong("id");//每个账单记录id
					  double receive_corpus = billInvestJson.getDouble("receive_corpus");//账单本金
					  double receive_interest = billInvestJson.getDouble("receive_interest");//账单利息
					  double billInvest_overdue_fine = billInvestJson.getDouble("overdue_fine");//账单逾期罚款
					  
					  DecimalFormat formater = new DecimalFormat();
				      formater.setMaximumFractionDigits(2);
				      formater.setGroupingSize(0);
				      formater.setRoundingMode(RoundingMode.DOWN); // 不四舍五入
				      
				      double max_interest=Double.parseDouble(formater.format(receive_corpus*repayment_interest/repayment_corpus*Constants.INTEREST_TIMES));
				      double check_overdue_fine=overdue_fine-sum_overdue_fine;
				      
				      if(receive_interest+billInvest_overdue_fine==max_interest){
				    	  continue;
				      }
				      
				      if(max_interest-receive_interest-billInvest_overdue_fine<=check_overdue_fine){
						  
				    	  check_overdue_fine=check_overdue_fine+receive_interest+billInvest_overdue_fine-max_interest;
				    	  billInvest_overdue_fine=max_interest-receive_interest;
					  }else{
						  
						  billInvest_overdue_fine=billInvest_overdue_fine+check_overdue_fine;
						  check_overdue_fine=0d;
					  }
				      
				      String updateSql = "update t_bill_invests set overdue_fine = ? where id = ?";
				 		
				      Query queryInvests = em.createQuery(updateSql).setParameter(1, billInvest_overdue_fine).setParameter(2, billInvestId);
				      
				      try {
				    	  int rows = queryInvests.executeUpdate();
				      } catch(Exception e) {
				    	  e.printStackTrace();
						  Logger.info("修改理财账单逾期生成的数据时："+e.getMessage());
						  error.code = -3;
...
// 		}
	...
	public static List<v_bill_loan> queryMyAllLoanBills(ErrorInfo error) {
 	 ...
 	 * @return List<v_bill_loan>
 	 */
	public static PageBean<v_bill_loan> queryMyLoanBills(long userId, long bidId,int payType, int isOverType,
	...
	/**
 	 ...
 	 * @return
 	 */
 	public static Double queryBillOutstand_amount_sum(String bid_id,String userId ,ErrorInfo error){
 		error.clear();
 		Double sum=0d;
 		try {
 			EntityManager em = JPA.em();
            Query query = em.createNativeQuery("SELECT sum((( `bl`.`repayment_corpus` + `bl`.`repayment_interest` ) + `bl`.`overdue_fine` )) FROM `t_bills` `bl` JOIN t_bids b on b.id=bl.bid_id WHERE ((`bl`.`bid_id` =?) AND b.user_id=? AND (`bl`.`status` IN(-(1) ,-(2))))");
            query.setParameter(1, bid_id);
 	...
 	public static PageBean<v_bill_repayment_record> queryBillReceivables(long bidid, int currPage, int pageSize, ErrorInfo error){
		...
		String sql="select new v_bill_repayment_record( a.current_pay_amount as current_pay_amount, a.title as title," +
				...
				"repayment_time )from v_bill_repayment_record as a where a.bid_id = ? order by a.id asc";
		try {
