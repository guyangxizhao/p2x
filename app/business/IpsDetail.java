...
public class IpsDetail {
	...
	public static String getIpsInfo(long pMerBillNo, ErrorInfo error) {
		error.clear();
		String sql = "select cache_info from t_ips_cache where p_mer_bill_no = ? order by id desc limit 1";
		
		String info = null;
		
		try{
			info = JPA.em().createNativeQuery(sql).setParameter(1, pMerBillNo).getResultList().get(0).toString();
		}catch(Exception e) {
