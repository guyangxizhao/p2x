...
import models.t_invest_jingtum;
...
import models.v_invest_jingtum;
...
public class Invest implements Serializable{
	private static final long serialVersionUID = -5716388578193428039L;
	private long _id;
	public long id;
	 ...
	 * @param _loanLmt 借款期限
	 * @param _loanInt 年化利率
	 * @param _loanSchedule 借款进度
	 * @param _repayType 还款方式
	 * @param _orderType 排序方式
	 * @param _keywords 输入的查询关键字
	 * @param error
	 * 
	 * @return
	 */
	@SuppressWarnings("unchecked")
	public static PageBean<v_front_all_bids> queryBidsByConditions(
			int showType, int currPage, int pageSize,
			String _loanType, String _loanLmt, String _loanInt,
			String _loanSchedule, String _repayType, String _orderType,
		...
		if (StringUtils.isBlank(_loanType) && StringUtils.isBlank(_loanLmt)
				&& StringUtils.isBlank(_loanSchedule) && StringUtils.isBlank(_loanInt)
				&& StringUtils.isBlank(_repayType)&& StringUtils.isBlank(_orderType)
		...
		if (NumberUtil.isNumericInt(_loanType)) {
			loanType = Integer.parseInt(_loanType);
			if (loanType > 0) {
		...
		// 借款期限
		if (NumberUtil.isNumericInt(_loanLmt)) {
			loanLmt = Integer.parseInt(_loanLmt);
		}
		if (loanLmt < 0 || loanLmt > 4) {
			sql.append(SQLTempletes.BID_LMT_CONDITION[0]); // 全部范围
		} else {
			if(loanLmt == 4){
		    sql.append("and t_bids.period_unit=-1 and t_bids.period>=1");	
			}else{
			sql.append(SQLTempletes.BID_LMT_CONDITION[loanLmt]);
			}	
		}

		// 年化利率
		if (NumberUtil.isNumericInt(_loanInt)) {
			loanInt = Integer.parseInt(_loanInt);
		}
		if (loanInt < 0 || loanInt > 4) {
			sql.append(SQLTempletes.BID_APR_CONDITION[0]); // 全部范围
		} else {
			sql.append(SQLTempletes.BID_APR_CONDITION[loanInt]);
		}
		
		// 认购进度
		if (NumberUtil.isNumericInt(_loanSchedule)) {
			loanSchedule = Integer.parseInt(_loanSchedule);
		}
		if (loanSchedule < 0 || loanSchedule > 4) {
			sql.append(SQLTempletes.BID_LOAN_SCHEDULE_CONDITION[0]);
		} else {
			sql.append(SQLTempletes.BID_LOAN_SCHEDULE_CONDITION[loanSchedule]);
		}
		
		// 还款方式
		if (NumberUtil.isNumericInt(_repayType)) {
			repayType = Integer.parseInt(_repayType);
		}
		if (repayType <= 0 || repayType > 4) {
			sql.append(SQLTempletes.BID_RPY_CONDITION[0]);
		} else {
			sql.append(SQLTempletes.BID_RPY_CONDITION[1]);
		...
		conditionMap.put("loanLmt", loanLmt);
		conditionMap.put("loanInt", loanInt);
		conditionMap.put("loanSchedule", loanSchedule);
		conditionMap.put("repayType", repayType);
	 ...
	 * @return
	 */
	public static PageBean<v_front_all_bids> queryAllBids(int showType, int currPage,
			int pageSize, String _apr, String _amount, String _loanSchedule, String _startDate,
			String _endDate, String _loanType, String minLevelStr, String maxLevelStr, 
			String _orderType, String _keywords, ErrorInfo error) {
		int apr = 0;
		int amount = 0;
		int loan_schedule = 0;
		int orderType = 0;
	...
	public static long queryTotalInvestCount(ErrorInfo error) {
		...
		String sql = "SELECT investCount,transferCount FROM((SELECT COUNT(id) AS investCount FROM t_invest_transfers WHERE `status` = ?) AS investCount,(SELECT COUNT(a.id) AS transferCount FROM t_invests a left join t_bids b on a.bid_id=b.id WHERE b.`status` IN (?, ?, ?, ?, ?, ?)) AS transferCount)";
	...
	public static double queryTotalDealAmount(ErrorInfo error) {
		...
		String sql = "SELECT transferAmount,investAmount FROM((SELECT SUM(debt_amount) AS transferAmount FROM t_invest_transfers WHERE `status` = ?) AS transferAmount,(SELECT SUM(a.amount) AS investAmount FROM t_invests a left join t_bids b on a.bid_id=b.id WHERE b.`status` IN (?, ?, ?, ?, ?, ?)) AS investAmount)";
		Query query = JPA.em().createNativeQuery(sql).setParameter(1, Constants.DEBT_SUCCESS).setParameter(2, Constants.BID_ADVANCE_LOAN).setParameter(3, Constants.BID_FUNDRAISE).setParameter(4, Constants.BID_EAIT_LOAN).setParameter(5, Constants.BID_REPAYMENT).setParameter(6, Constants.BID_REPAYMENTS).setParameter(7, Constants.BID_COMPENSATE_REPAYMENT);
	...
	public static void invest(long userId, long bidId, int investTotal, String dealpwdStr, boolean isAuto, boolean isRepair, String repairBillNo, ErrorInfo error) {
		...
		if (is_deal_password == true && !isRepair) {
			...
			if (!Encrypt.MD5(dealpwdStr + Constants.ENCRYPTION_KEY,Charset.forName("utf-8")).equals(
					dealpwd)) {
		...
		if (Constants.IPS_ENABLE && !isRepair) {
			...
			IpsDetail.setIpsInfo(Convert.strToLong(pMerBillNo, 0), JSONObject.fromObject(map).toString(), error);
			...
			if(StringUtils.isEmpty(str) || !str.trim().startsWith("{")){
				Logger.info("自动投标接口输出参数not json:%s" + str);
				return;
			}
			...
			JSONObject jsonPara = (JSONObject)Converter.xmlToObj(p3DesXmlPara);
			
			EntrustLogs.addCallbackDetail(error, Convert.strToLong(pMerBillNo.trim(), 0), pMerCode, pErrCode, pErrMsg, jsonObj.getString("p3DesXmlPara"), pSign, jsonPara);
			...
			pFee = jsonPara.getDouble("pFee");
			pIpsBillNo = jsonPara.getString("pP2PBillNo");  
	 ...
	 * @param error
	 */
	public static void doInvest(t_users user1, Map<String, String> bid, double investTotal, String pMerBillNo, double pFee, ErrorInfo error) {
		error.clear();
...
//		Map<String, Double> funds = DealDetail.queryUserFund(userId, error);
	...
	/*	DealDetail dealDetail = new DealDetail(userId,
		...
		dealDetail.addDealDetail(error);*///造成交易记录列表重复显示，暂时去掉
		...
		/**
		 * 满标 之后 ，根据t_user_details 记录  进行
		 *资金归结
		 *t_invests   投资详情
		 *bidid  标id
		 *invest_amount  当前用户的投资金额
		 *需要转账 的金额  资金归结 到p2x 银关
		 */
		Logger.info("amount:"+amount);
		Logger.info("hasInvestedAmount + investTotal:"+hasInvestedAmount + investTotal);
		if (amount == (hasInvestedAmount + investTotal)) {
			List<v_invest_jingtum> info=new ArrayList<v_invest_jingtum>();
			info=JPA.em().createNativeQuery("select * from v_invest_jingtum where bid_id = ?0",v_invest_jingtum.class).setParameter("0", bidId).getResultList();
			Jingtum  jing=new Jingtum();
			for (int i = 0; i < info.size(); i++) {
				t_invest_jingtum entity=new t_invest_jingtum();
				entity.bid_id=info.get(i).bid_id;
				entity.bid_address=info.get(i).bid_address;
				entity.create_time=new Date();
				entity.invest_amount=info.get(i).invest_amount;
				entity.wallet_address=info.get(i).wallet_address;
				entity.wallet_secret=info.get(i).wallet_secret;
				entity.user_id=info.get(i).user_id;
				
				try {
					Logger.info("wallet_address:"+info.get(i).wallet_address);
					Logger.info("invest_amount:"+info.get(i).invest_amount);
					Logger.info("bid_address:"+info.get(i).bid_address);
					boolean bool=jing.buyRZTong(//井通转账  从投资人转账到银关
							info.get(i).wallet_address,
							info.get(i).wallet_secret,
							info.get(i).invest_amount, "",info.get(i).bid_address,"");
					if(!bool){
						//井通转账失败
						entity.status=false;
						error.msg = "对不起！系统异常！资金归结失败！";
						error.code = -8;
						return ;
					}else{
						entity.status=true;
	...
	public static void automaticInvest() {
		...
		for(Map<String, Object> map : biderList) {
			...
			for(Long userId2 : userIds) {
				...
				try {
					invest(userId2, bidId, bidAmount, "", true, false, null, error);
				} catch (Exception e) {
