...
import jingtum.JingtumCallException;
...
import models.t_dict_industries;
...
import yeepay.YeepayInterfaces;
...
public class User extends UserBase implements Serializable{
	private static final long serialVersionUID = -1882961845558959203L;
...
//		this.isActive = user.isActive;  //是否是活跃 会员
		...
		this.industryName = user.industry_name;
	...
	public void setName(String name) {
		...
		try {
			...
			sql.append(SQLTempletes.SELECT);
			sql.append(SQLTempletes.V_USERS);
			sql.append(" and t_users.name = ? or t_users.email = ? or t_users.mobile = ?");
			EntityManager em = JPA.em();
            Query query = em.createNativeQuery(sql.toString(),v_users.class);
	...
	public void setPassword(String password) {
		this._password = Encrypt.MD5(password + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
	}
	...
	public void setPayPassword(String payPassword) {
		this._payPassword = Encrypt.MD5(payPassword + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
	}
...
//	}
	...
	public int register(ErrorInfo error){ 
		...
		try {
			Qrcode.create(this.getSpreadLink(), BarcodeFormat.QR_CODE, 100,
					100, new File(Blob.getStore(), uuid).getAbsolutePath(),
					"png");
		} catch (Exception e) {
			e.printStackTrace();
		...
		String[] result=new String[2];//井通钱包地址
			Jingtum jingapi=new Jingtum();
		try {
			result=jingapi.register();
			if(result!=null && result[0] != null && result!=null){
				user.wallet_address=result[0];
				user.wallet_secret=result[1];
				this.wallet_address=result[0];
				this.wallet_secret=result[1];
				
			}else{
				Logger.info("注册时，注册钱包失败！");
				error.code = -5;
				error.msg = "注册时，注册钱包失败！";
				return error.code;
			}
		} catch (JingtumCallException e) {
			e.printStackTrace();
			Logger.info("注册钱包："+e.getMessage());
			error.code = -5;
			error.msg = "对不起，由于平台出现故障，此次注册失败！";
		...
		try {
			if(result!=null && result[0] != null && result!=null){
				String[] active=jingapi.activity(result[0]);
				boolean bool=jingapi.setTrust(result[0], result[1],"", Constants.CURRENCY_CNY, Constants.CNY_COUNTERPARTY);
				if(active==null ||!"true".equals(active[3])||!"tesSUCCESS".equals(active[7])||!bool){//激活钱包
					Logger.info("注册时，激活钱包失败！");
					error.code = -13;
					error.msg = "注册时，激活钱包失败！";
					return error.code;
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
			Logger.info("注册钱包激活时："+e.getMessage());
			error.code = -14;
			error.msg = "对不起，由于平台出现故障，此次注册失败！";
			return error.code;
		}
		...
		this.id = user.id;
		String sign1 = Encrypt.MD5(""+this._id + 0.00 + 0.00 + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
		
		if(Constants.IPS_ENABLE) {
			sign1 = Encrypt.MD5(""+this._id + 0.00 + 0.00 + 0.00 + Constants.ENCRYPTION_KEY);
		}
		
		String sign2 = Encrypt.MD5(""+this._id + 0.00 + 0.00 + 0.00 + 0.00 + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
	...
	public int login(String password,boolean encrypt, ErrorInfo error){
		...
		if(!encrypt){
			//密码传入为明文则进行加密处理
			password = Encrypt.MD5(password+Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
		}
	 ...
	 * @return
	 */
	public void edit(User newUser, ErrorInfo error){
		error.clear();
...
//		}
		...
		if(newUser.educationId == 0) {
			error.code = -1;
			error.msg = "请选择最高学历";
			return;
		}
		
		if(newUser.maritalId == 0) {
			error.code = -1;
			error.msg = "请选择婚姻情况";
			return;
		}
		
		if(StringUtils.isBlank(newUser.familyAddress)) {
			error.code = -1;
			error.msg = "请输入居住地址";
			return ;
		}
...
//		}
		...
		if(newUser.houseId == 0) {
			error.code = -1;
			error.msg = "请选择住房条件";
			return;
		}
		
		if(newUser.industry == 0) {
			error.code = -1;
			error.msg = "请选择所在行业";
			return;
		}
...
//		}
		...
		Query query = em.createQuery("update t_users set name=?,education_id=?,graduate=?,marital_id=?,"
				+ "family_address=?,house_id=?,industry=?,income=?,is_add_base_info=? where id=?")
				.setParameter(1, newUser.name)
				.setParameter(2, newUser.educationId)
				.setParameter(3, newUser.graduate)
				.setParameter(4, newUser.maritalId)
				.setParameter(5, newUser.familyAddress)
				.setParameter(6, newUser.houseId)
				.setParameter(7, newUser.industry)
				.setParameter(8, newUser.income)
				.setParameter(9, Constants.TRUE)
				.setParameter(10, getId());
	...
	public Model refreshUserInfo(long userId) {
		Model user = null;
		try{
			StringBuffer sql = new StringBuffer();
			List<v_users> v_users_list = null;
	...
	public int addPayPassword(boolean flag, String payPassword1, String payPassword2, ErrorInfo error) {
		...
		String payPassword = Encrypt.MD5(payPassword1+Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
	...
	public int editPayPassword(String oldPayPassword, String newPayPassword1, String newPayPassword2, ErrorInfo error) {
		...
		String oldPassword = Encrypt.MD5(oldPayPassword+Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
		String payPassword = Encrypt.MD5(newPayPassword1+Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
	...
	public int verifyPayPassword(String payPassword, ErrorInfo error) {
		...
		if(!this._payPassword.equalsIgnoreCase(Encrypt.MD5(payPassword + Constants.ENCRYPTION_KEY,Charset.forName("utf-8")))) {
			error.code = -1;
	...
	public int editPassword(String oldPassword, String newPassword1, String newPassword2, ErrorInfo error){
		...
		if(!Encrypt.MD5(oldPassword + Constants.ENCRYPTION_KEY,Charset.forName("utf-8")).equalsIgnoreCase(this._password)) {
			error.code = -2;
			error.msg = "密码错误！";
		...
		Query query = em.createQuery(sql).setParameter(1, Encrypt.MD5(newPassword1 + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"))).setParameter(2, this._id);
...
if (Constants.CHECK_MSG_CODE) {
			...
			if (StringUtils.isBlank(randomID) || StringUtils.isBlank(Cache.get(randomID).toString())) {
				error.code = -1;
		...
		EntityManager em = JPA.em();
		String sql = "update t_users set password = ? where id = ?";
		Query query = em.createQuery(sql).setParameter(1, Encrypt.MD5(password + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"))).setParameter(2, userId);
	...
	public void updatePasswordByEmail(String password, String confirmPassword, ErrorInfo error){
		...
		EntityManager em = JPA.em();
		String sql = "update t_users set password = ? where id = ?";
		Query query = em.createQuery(sql).setParameter(1, Encrypt.MD5(password + Constants.ENCRYPTION_KEY,Charset.forName("utf-8"))).setParameter(2, this.id);
	...
	public void updatePayPasswordByEmail(String password, String confirmPassword, ErrorInfo error){
		...
		String payPassword = Encrypt.MD5(confirmPassword+Constants.ENCRYPTION_KEY,Charset.forName("utf-8"));
...
//		}
//		
		String sql = "update t_users set email = ?, is_email_verified = 1 where id = ?";
	...
	public void savenewemail(){
		String sql = "update t_users set var_email = ? where id = ?";
		int rows=0;
		rows = JpaHelper.execute(sql, this.email, this.id).executeUpdate();
	}
	 ...
	 * @throws Exception 
	 */
	public void saveBirthday(String sex,String brithday) throws Exception{
		SimpleDateFormat formatter = new SimpleDateFormat( "yyyy-MM-dd");
	    Date date = formatter.parse(brithday);
	    String sql = "update t_users set sex = ?, birthday = ? where id = ?";
	int rows=JpaHelper.execute(sql, Integer.parseInt(sex), date, this.id).executeUpdate();	
	...
	public static int isIDNumberExist(String idNumber, ErrorInfo error){
		...
		if(name != null && idNumber.equals(name)) {
			error.code = -2;
	 ...
	 * @return
	 */
	public static int isMobileNotExist(String userMobile, ErrorInfo error){
		error.clear();
...
//		}
	...
	public static String encrypt(){
		...
		return com.shove.security.Encrypt.MD5(all.trim(),Charset.forName("utf-8"));
	}
...
//		}
	...
	public long withdrawal(double amount, long bankId, String payPassword, int type, boolean flag, ErrorInfo error){
		...
		if (!(Constants.IPS_ENABLE && flag)) {
			...
			if(!Encrypt.MD5(payPassword+Constants.ENCRYPTION_KEY,Charset.forName("utf-8")).equalsIgnoreCase(this.payPassword)) {
				error.code = -1;
	...
	public static void withdrawalNotice(long userId, double serviceFee,
		...
		if(Constants.IPS_ENABLE && Constants.IS_WITHDRAWAL_AUDIT && !flag) {
			...
			/* 伪构记录 */
			/*DealDetail _detail = new DealDetail(userId,
					DealType.PAY_FREEZE_FUND, amount, withdrawalId, balance,
					freeze - amount, receiveAmount, "支付冻结提现金额");*/
			...
			Query query = JpaHelper.execute("update t_user_withdrawals set status = 4, audit_supervisor_id = ?,"
					+ " pay_time = ? where id = ? and status = 3", -1L, new Date(),
					withdrawalId);
...
//				Query query = JpaHelper.execute("update t_user_withdrawals set status = 2, audit_supervisor_id = ?,"
//						+ " pay_time = ? where id = ? and status = 1", supervisorId, new Date(),
//						withdrawalId);
//				
//				//资金托管
//				if(Constants.IPS_ENABLE && Constants.IS_WITHDRAWAL_AUDIT && !flag) {
//					//资金托管，提现需审核，且返回状态码为M00010F（处理中）
...
//				}
		...
		if (Constants.IPS_ENABLE && flag && !(Constants.IS_WITHDRAWAL_AUDIT && withdrawal.status == 4)) {
			balance = balance - amount;
		} else{
...
//		}
	...
	public static t_users queryUserByUserId2(long userId,ErrorInfo error){
		error.clear();
		System.out.println("数据查询中");
		String sql = "select new t_users(name,email,var_email,id) from t_users where id = ?";
		t_users user = null;
	 ...
	 * @return
	 */
	public static long queryIdByEmail(String email, ErrorInfo error) {
		error.clear();
		
		String sql = "select id from t_users where email = ?";
		List<Long> ids = null;
		
		try {
			ids = t_users.find(sql, email).fetch();
		}catch(Exception e) {
			e.printStackTrace();
			Logger.info("根据用户名查询id时，根据用户名查询用户ID时："+e.getMessage());
			error.code = -1;
	...
	public static void queryBasic() {
		...
		List<t_dict_industries> industries = null;
		List<t_dict_banks> banks = null;
		...
		try {
			...
			industries = t_dict_industries.findAll();
			banks = t_dict_banks.findAll();
		} catch (Exception e) {
			e.printStackTrace();
...
//		conditionMap.put("type", type);
	 ...
	 * @return
	 */
	public static List<v_user_deals_record> queryAllDealRecord(ErrorInfo error) {
		error.clear();
		
		List<v_user_deals_record> details = new ArrayList<v_user_deals_record>();
		StringBuffer sql = new StringBuffer("");
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_USER_DEALS_RECORD);
		sql.append(" and t_users.id = ? ");
		sql.append(SQLTempletes.V_USER_DETAILS_GROUP_ORDER);
		...
		try {
			EntityManager em = JPA.em();
            Query query = em.createNativeQuery(sql.toString(),v_user_deals_record.class);
            for(int n = 1; n <= params.size(); n++){
                query.setParameter(n, params.get(n-1));
            }
            details = query.getResultList();
	...
	public static PageBean<v_user_deals_record> queryUserDealRecord(long userId, long type, String beginTime,
			String endTime, int currPage, int pageSize) {
		if(currPage == 0) {
			currPage = 1;
		}
		...
		PageBean<v_user_deals_record> page = new PageBean<v_user_deals_record>();
		page.currPage = currPage;
		page.pageSize = pageSize;
		page.conditions = conditionMap;
		
		List<v_user_deals_record> userDetails = new ArrayList<v_user_deals_record>();
	 ...
	 * @return
	 */
	public static Double pendingRepaymentAmountByBillId(long userId,long billId,ErrorInfo error){
		
		Double count = 0.0;
			
		String	sql = "select sum(repayment_corpus + repayment_interest +overdue_fine) from t_bills where bid_id = "+billId+"  and status in (-1,-2)";
	...
	public static Map<String ,String> historySituation(long userId,ErrorInfo error){
        ...
        historySituationMap.put("loanAmount",  df.format(loanAmount));//借入金额
	 ...
	 * @return
	 */
	public static Map<String, String> yeePay(BigDecimal money, int bankType, int type, boolean isApp, ErrorInfo error){
		error.clear();
		...
		t_dict_payment_gateways gateway = gateway(Constants.YEE_GATEWAY, error);
		
		if(error.code < 0) {
			return null;
		}
		String billno = NumberUtil.getBillNo(type);
	    YeepayPayment yee=new YeepayPayment(
	    		gateway._key,
	    		Constants.YEE_URL,
	    		gateway.account,
	    		billno,
	    		money.toString(),
	    		Constants.YEE_P5_PID,
	    		Constants.YEE_P6_PCAT,
	    		Constants.YEE_P7_PDESC,
	    		Constants.YEE_MERCHANT_URL,
	    		Constants.YEE_PA_PM,
	    		Constants.YEE_PD_FRPID);
	    
	    sequence(3, billno, money.doubleValue(), Constants.GATEWAY_RECHARGE, error);
	    
	    return YeepayInterfaces.objToMap(yee);
	}
	/**
	 * 井通支付接口
	 * @param money
	 ...
	 * @return
	 */
	public static Map<String, String> jingPay(BigDecimal money, int bankType, int type, boolean isApp, ErrorInfo error){
		error.clear();
		if(bankType < 0 || bankType > 5) {
			error.code = -1;
			error.msg = "传入参数有误";
		...
		t_dict_payment_gateways gateway = gateway(Constants.YEE_GATEWAY, error);//易宝支付
		
		if(error.code < 0) {
			return null;
		}
		t_users user=t_users.findById(Cache.get("front_"+Session.current().getId()));
	
		String billno = NumberUtil.getBillNo(type);
		StringBuffer string=new StringBuffer();
		string.append(Constants.JING_P0_CMD);
		string.append(gateway.account);//易宝支付
		string.append(billno);
		string.append(user.wallet_address);
		string.append(Arith.round(money.doubleValue(), 4)+"");
		string.append(Constants.CURRENCY_CNY);
		string.append(Constants.JING_P6_PRODUCT);
		string.append(Constants.JING_P7_CATALOG);
		string.append(Constants.JING_P8_DESC);
		string.append(Constants.JING_P9_URL);
		string.append(Constants.JING_PA_EXT);
		string.append(Constants.JING_PB_PASS);
		string.append(Constants.JING_PC_PERIOD);
		string.append(Constants.JING_PD_UNIT);
		string.append(Constants.JING_PE_NEED);
		
		String hmac= Hmac.getHmacMd5Bytes(string.toString(),Constants.JING_HMAC_KEY).toLowerCase();
		
		Map<String, String>  map=new HashMap<String, String>();
		map.put("jing_url", Constants.JING_URL);
		map.put("p0_cmd", Constants.JING_P0_CMD);
		map.put("p1_custom", gateway.account);
		map.put("p2_order", billno);
		map.put("p3_account", user.wallet_address);
		map.put("p4_amount", Arith.round(money.doubleValue(), 4)+"");
		map.put("p5_currency", Constants.CURRENCY_CNY);
		map.put("p6_product", Constants.JING_P6_PRODUCT);
		map.put("p7_catalog", Constants.JING_P7_CATALOG);
		map.put("p8_desc", Constants.JING_P8_DESC);
		map.put("p9_url", Constants.JING_P9_URL);
		map.put("pa_ext", Constants.JING_PA_EXT);
		map.put("pb_pass", Constants.JING_PB_PASS);
		map.put("pc_period", Constants.JING_PC_PERIOD);
		map.put("pd_unit", Constants.JING_PD_UNIT);
		map.put("pe_need", Constants.JING_PE_NEED);
		map.put("hmac", hmac);
		
		sequence(4, billno, money.doubleValue(), Constants.GATEWAY_RECHARGE, error);
	    return map;//MAP
	}
	/**
	 * 所有支付接口
...
//		String gopayServerTime = GopayUtils.getGopayServerTime();
	...
	public static void goffSingle(String payNumber,t_user_recharge_details user_recharge,ErrorInfo error){
		...
		User.recharge(payNumber, Double.parseDouble(orgtranAmt), error);
	...
	public static void rewardCPS(long userId,double managefee,long relationId,ErrorInfo error){
		...
		if(CPStype == Constants.CPSREWARDTYPE_USER_COUNT){//1:-按会员数
		   ...
		   Map<String, Double> funds = DealDetail.queryUserFund(recommendUserId, error);
			...
			double award =Arith.round(Arith.mul(rewardForRate/100, managefee), 2);
	...
	public static t_users queryUserforInvest(long userId, ErrorInfo error) {
		...
		String sql = "select id ,name , balance ,is_blacklist ,pay_password, master_identity, mobile, email from t_users  where id = ? ";
		Object [] obj = null;
		...
		user.email= null == obj[7] ? "" : obj[7].toString();
	...
	public static boolean updateMobileVerified(long userId) {
		String hql = "update t_users set is_mobile_verified=1 where id=?";
		Query query = JPA.em().createQuery(hql).setParameter(1, userId);
		...
		try {
			...
			JPA.setRollbackOnly();
			Logger.error("更新手机验证状态发生异常：\n" + e.getMessage());
		}
		return rows > 0;
	}
	public static User queryUserbyIdisDetail(long id){
//		t_users t_user=null;
//		
//		String sql1 = "select new t_users(age, id) from t_users where id = ?";
//		t_users user = null;
//		user = t_users.find(sql1, id).first();
		
		//
		StringBuffer sql = new StringBuffer();
         List<v_users> list=null;
		v_users v=null;
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_USERS);
		sql.append(" and t_users.id = ?");
        ...
        v=list.get(0);
        User user=new User();
        user.realityName=v.reality_name;
        if(v.sex==1){
        user.sex="男";
        }else if(v.sex==2){
         user.sex="女";
        ...
        user.idNumber=v.id_number;
        user.educationName=v.education_name;
        user.maritalName=v.marital_name;
        user.houseName=v.house_name;
        user.provinceName=v.province_name;
        user.carName=v.car_name;
	 ...
	 * @return
	 */
	public boolean needCompleteBasicInfo() {
		User user = currUser();
		if (user == null)
			return true;
		
		return StringUtils.isNotBlank(user.realityName)
			&& StringUtils.isNotBlank(user.mobile)
			&& user.isMobileVerified
			&& StringUtils.isNotBlank(user.email)
			&& user.isEmailVerified
			&& (user.educationId > 0)
			&& (user.maritalId > 0)
			&& StringUtils.isNotBlank(user.familyAddress)
			&& (user.houseId > 0)
			&& (user.industry > 0)
			&& (user.income > 0);
	}
	 ...
	 * @return
	 */
	public boolean needCompleteSafeInfo() {
		User user = currUser();
		if (user == null)
			return true;
		...
		return StringUtils.isNotBlank(user.realityName)
				...
				&& StringUtils.isNotBlank(user.email)
				&& user.isEmailVerified
				&& StringUtils.isNotBlank(user.payPassword);
	}
}
