...
public class Bid implements Serializable{
	private static final long serialVersionUID = 1076502855412785759L;
	public long id;
	private long _id;
	public String bidNo;
	...
	public String getStrStatus() {
		if(null == this.strStatus){
			switch (this.status) {
				...
				case Constants.BID_JINGTUMAUDIT:this.strStatus="井通审核中";break;
...
//			this.bidNo  = this.bidNo.substring(this.bidNo.length() - 14);
	...
	public void afterCreateBid(t_bids tbid, String bidNo, ErrorInfo error) {
		...
		dealDetail.addDealDetail(error);*///这里交易记录查询重复添加了，暂时去掉
		...
		/* 添加事件 */
		if(tbid.is_agency)
			DealDetail.supervisorEvent(this.userId, SupervisorEvent.CREATE_AGENCY_BID, "管理员发布合作机构标", error);
		else
	...
	public void auditToFundraise(ErrorInfo error){
		...
		query.setParameter(4, Constants.BID_JINGTUMAUDIT);// 审核状态为井通资源审核中
		...
		try {
			...
			e.printStackTrace();
			Logger.error("标->审核中->井通资源审核:" + e.getMessage());
			error.msg = "修改状态失败!";
			JPA.setRollbackOnly();
		...
		this.status = Constants.BID_JINGTUMAUDIT; //审核状态为井通资源审核中
		
		/* 4.向井通申请发行融资通*/
		//默认为手动
		addP2xTum(_id, amount, error);
		if(error.code < 0){
			error.msg = "审核失败!";
			JPA.setRollbackOnly();
			return;
		...
		/* 5.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：审核中->井通审核中", error);
		if(error.code < 0){
			error.msg = "添加事件失败!";
			JPA.setRollbackOnly();
			return;
	...
	public void advanceLoanToFundraise(ErrorInfo error){
		...
		try {
			...
			e.printStackTrace();
			Logger.error("标->提前借款->井通审核中:" + e.getMessage());
			error.msg = "审核失败!";
			JPA.setRollbackOnly();
		...
		/* 4.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：提前借款->井通审核中", error);
	 ...
	 * 井通审核中【id:15】->筹款中
	 */
	public void jingtumToFundraise(ErrorInfo error){
		error.code = -1;
		int row = 0;
		
		/* 1.确定当前bid的状态 */
		if(Constants.BID_JINGTUMAUDIT != this.status){
			error.msg = "非法审核!";
...
//		query.setParameter(2, new Date());// 审核时间
//		query.setParameter(3, this.auditSuggest);// 审核意见
//		query.setParameter(4, Constants.BID_FUNDRAISE);// 审核状态为井通资源审核中
//		query.setParameter(5, this.id); // 标ID
//		query.setParameter(6, this.userId); // 用户ID
//		query.setParameter(7, Constants.BID_JINGTUMAUDIT); // 当前状态必须为提前借款
//
//		try {
//			row = query.executeUpdate();
//		} catch (Exception e) {
//			e.printStackTrace();
//			Logger.error("标->井通审核中->审核通过:" + e.getMessage());
//			error.msg = "审核失败!";
//			JPA.setRollbackOnly();
//
//			return;
...
//		}
		...
		/* 4.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：井通审核中->筹款中->", error);
		...
		/* 5.修改井通审核状态 为1【通过】*/
		 ...
		 update.append("update t_bid_jingtum set  result = 1 where bid_id = ");
		 update.append(this.id);
	 ...
	 * @param error
	 */
	public void jingtumToNotThrough(ErrorInfo error) {
		error.clear();
		int row = 0;
		...
		query.setParameter(5, this.id); // 标ID
		query.setParameter(6, this.userId); // 用户ID
		query.setParameter(7, Constants.BID_JINGTUMAUDIT); // 当前状态必须为审核中

		try {
			row = query.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
			Logger.error("标->井通审核中->审核不通过:" + e.getMessage());
			error.msg = "审核失败!";
			JPA.setRollbackOnly();
			return;
		}
		...
		/* 4.添加事件 */
		DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：井通审核中->审核不通过", error);
	...
	public void eaitLoanToRepayment(ErrorInfo error){
		...
		if (Constants.IPS_ENABLE && !this.isRepair) {
			...
			if("1.0".equals(BackstageSet.getCurrentBackstageSet().entrustVersion) && this.bail>0) {
				if (!Payment.guaranteeUnfreeze(this, error)) {
					return;
				}
			}
	...
	public void doEaitLoanToRepayment(ErrorInfo error){
		...
		jing.transferToBorrowers(Constants.P2X_ADDRESS, Constants.P2X_SECRET, this.amount+"", user.wallet_address, Constants.CURRENCY_CNY, Constants.CURRENCY_CNY);
	...
	private void investUserAmountProcess(ErrorInfo error){
		...
		for (int i = 0; i < invests.size(); i++) {
			...
			if(this.bonusType != Constants.NOT_REWARD){
				...
				if (Constants.PAY_TYPE_FUND == PayType.INDEPENDENT || Constants.PAY_TYPE_FUND == PayType.SHARED||(Constants.IPS_ENABLE && Constants.PAY_TYPE_FUND == PayType.INNER)) {
					row = DealDetail.addUserFund2(invest.investUserId, fund); // 增加用户的金额
				} else if( Constants.IPS_ENABLE && Constants.PAY_TYPE_FUND == PayType.INNER){
					row = DealDetail.addUserFund2(invest.investUserId, fund); // 增加用户的金额
				} else {
	...
	public List<Map<String, Object>> queryInvestFunds(ErrorInfo error) {
		...
		for (int i = 0; i < invests.size(); i++) {
			...
			if(IPSConstants.CurrentGateWay == IPSConstants.GATE_WAY_PNR){  //汇付
				map.put("ordId", Payment.createBillNo(0L, 0));
			}
...
//	}
	 ...
	 * @return List<v_bid_auditing>
	 */
	public static List<v_bid_auditing> queryBidAuditingToFront(PageBean<v_bid_auditing> pageBean, ErrorInfo error, String ... str) {
		error.clear();
		int count = -1;
		StringBuffer conditions = new StringBuffer
				(" where 1 = 1"); 
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select count(t.id) from (");
		sql.append(SQLTempletes.V_BID_AUDITING2);
		sql.append(")");
		sql.append(SQLTempletes.TABLE_NAME);
		sql.append(conditions);
		EntityManager em = JPA.em();
	 ...
	 * @return
	 */
	public static List<v_bid_JingTum> queryJingTumState(PageBean<v_bid_JingTum> pageBean, ErrorInfo error, String ... str) {
		error.clear();
		int count = -1;
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select count(t.id) from (");
		sql.append(SQLTempletes.V_BID_JINGTUMING);
		sql.append(") ");
		sql.append(SQLTempletes.TABLE_NAME);
		sql.append(conditions);
        ...
        try {
			...
			Logger.error("标->井通审核中的标列表,查询总记录数:" + e.getMessage());
			error.msg = "加载筹款中的借款标列表失败!";
		...
		if(count < 1)
			return new ArrayList<v_bid_JingTum>();
		...
		try {
			...
			Logger.error("标->井通审核中的标列表,查询列表:" + e.getMessage());
			error.msg = "加载审核中的借款标列表失败!";
	...
	public static class Repayment implements Serializable{
		...
		public static Repayment queryRepaymentById(long id, ErrorInfo error){
			t_dict_bid_repayment_types t=new t_dict_bid_repayment_types();
			String sql="select new t_dict_bid_repayment_types(id, name, is_use) "
					+ "from t_dict_bid_repayment_types where id=?";
			t=t_dict_bid_repayment_types.find(sql, id).first();
			Repayment repayment=new Repayment();
			repayment.id=t.id;
			repayment.name=t.name;
			repayment.isUse=t.is_use;
			return repayment ;
	 ...
	 * @return
	 */
	public void _setBidForJingtum(String bid_id,int state,ErrorInfo error){
			error.msg="";
			error.code=0;
			...
			query.setParameter(5, this.id); // 标ID
			query.setParameter(6, this.userId); // 用户ID
			query.setParameter(7, Constants.BID_JINGTUMAUDIT); // 当前状态必须为井通审核中

			try {
				row = query.executeUpdate();
			} catch (Exception e) {
				e.printStackTrace();
				Logger.error("标->井通审核中->审核通过-筹款中:" + e.getMessage());
				error.msg = "修改状态失败!";
				JPA.setRollbackOnly();
			...
			this.status = Constants.BID_FUNDRAISE; //审核状态为井通资源审核中
			/* 4.向井通申请发行融资通*/
			//默认为手动
			
			/* 5.添加事件 */
			DealDetail.supervisorEvent(this.allocationSupervisorId, SupervisorEvent.AUDIT_BID, "审核标：井通审核中->筹款中[审核通过]", error);
			if(error.code < 0){
				error.msg = "添加事件失败!";
				JPA.setRollbackOnly();
				return;
	 ...
	 * @return
	 */
	public  boolean addP2xTum(long bid,double amount,ErrorInfo error) {
		error.clear();
		Jingtum jing=new Jingtum();
		try {
			String[] tmp=jing.forJingTumBid(String.valueOf(bid),String.valueOf(amount));
			String bid_id=tmp[0];
			String bid_address=tmp[1];
			String startDate=tmp[2];
			t_bid_jingtum bid_tum=new t_bid_jingtum();
			bid_tum.bid_id=Integer.parseInt(bid_id);
			bid_tum.bid_address=bid_address;
			bid_tum.start_time=new Date(Long.parseLong(startDate));
			bid_tum.save();
 			return true;
		} catch (JingtumCallException e) {
			Logger.error("申请融资通失败：%s", e.getMessage());
			error.code=-1;
			error.msg="申请发行标失败";
			JPA.setRollbackOnly();
			return  false;
	...
	/**
	 * 获取井通对标的审核状态，
	 * 成功：p2x银关内存在 以标id为currency的货币记录
	 */
	public static void getP2xTumStatus(){
		Logger.info("-----------井通审核状态查询开始---------");
		StringBuffer sql = new StringBuffer();
		sql.append(SQLTempletes.SELECT);
		sql.append(SQLTempletes.V_BID_JINGTUM);
		sql.append(" FROM t_bid_jingtum where result =1 and status =0");  //处理  通过申请 并且未发融资通的标
		EntityManager em = JPA.em();
        Query query = em.createNativeQuery(sql.toString(),t_bid_jingtum.class);
        List<t_bid_jingtum> list = query.getResultList();
        //井通未处理的所有的标
        Jingtum jing=new Jingtum();
         try {
        	...
        	// Arrays.toString(tmp.toArray());
        	 for (int i = 0; i < list.size(); i++) {
        		 Bid bid=new Bid();
        		 t_bid_jingtum t_bid =list.get(i);
        		 if(jing.getJingTumStatus(t_bid.bid_address)){//井通已经审批
        			tmp.add(t_bid.bid_address);
        			
        			bid.setId(t_bid.bid_id);
        			 
        			String hql = "update t_bids set audit_time=?, status=? where id= ? and status=?";
         			System.out.println("----"+Constants.BID_FUNDRAISE+"------"+bid.id+"-----"+Constants.BID_AUDIT);
        			Query upstate = JPA.em().createQuery(hql);
         			upstate.setParameter(1, new Date());// 审核时间
         			upstate.setParameter(2, Constants.BID_FUNDRAISE);// 审核状态为审核通过
         			upstate.setParameter(3, Long.parseLong(bid.id+"")); // 标ID
         			upstate.setParameter(4, Constants.BID_JINGTUMAUDIT); // 当前状态必须为审核中
         			...
         			bid.addInvestExpireTime(t_bid.bid_id,bid.investPeriod); //修改满标期限
         			bid.auditToadvanceLoanNotice();
        		 }
        	 ...
        	 if(tmp.size() > 0){
        		 for (int i = 0; i < tmp.size(); i++) {
        			 StringBuffer update = new StringBuffer();
        			 update.append("update t_bid_jingtum set status = 1 , result = 1 , end_time =?");
        			 update.append(" where bid_address in ( ? )");
        			 Query rs = em.createQuery(update.toString());
        			 rs.setParameter(1, new Date());
        			 rs.setParameter(2, tmp.get(i));
        			 int change=rs.executeUpdate();
        			 if(change < 1){
        				 JPA.setRollbackOnly();
        				 Logger.error("修改井通审核状态失败");
			...
			Logger.error("修改井通审核状态错误：%s", e.getMessage());
		}
         Logger.info("-----------井通审核状态查询结束---------");
	}
	/**
	 * 查询标的信息用于投资
	 * @param bidId
	 ...
	 * @return
	 */
	public static Bid queryBidForDetails(long bidId, ErrorInfo error) {
		error.clear();
		String sql = "select id, user_id, bid_no, loan_purpose_id, amount, has_invested_amount, service_fees, "
				+ "apr, title, amount, period_unit, repayment_time, repayment_type_id, average_invest_amount, loan_schedule, min_invest_amount, status, product_id from t_bids  where id = ? limit 1";
		Object[] obj = null;
		try {
			obj = (Object[]) JPA.em().createNativeQuery(sql).setParameter(1, bidId).getSingleResult();
		} catch (Exception e) {
