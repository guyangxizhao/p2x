...
public class BackstageSet implements Serializable{
	private static final long serialVersionUID = -1737055215105628103L;
	...
	public String recruitmentPeriod;//是否开启募集期奖励
	public String interest;//募集期的天利率
	...
	public String smsAppid; // 短信通道AppID
	public String smsServer; // 短信通道服务器地址
	public String smsServPort; // 短信通道服务器端口
	public String smsTemplateId; // 短信通道短信模板ID
	public String smsTimeLimit; // 短信验证码有效期
	public long smsLimit; // 短信验证码有效期
	 ...
	 * @return
	 */
	public int settingRaiseInterest(ErrorInfo error){
		error.clear();
		int rows = 0;
		String sql = "update t_system_options set _value = case _key " +
                " when 'recruitmentPeriod' then ?" +
                " when 'interest' then ?" +
                " end where id in(10101, 10102)";
		try {
			/*绑定数据，执行语句*/
			rows = JPA.em().createNativeQuery(sql).setParameter(1, this.recruitmentPeriod)
				   .setParameter(2, this.interest).executeUpdate();
		...
		/*保存管理员操作记录*/
		DealDetail.supervisorEvent(Supervisor.currSupervisor().id, SupervisorEvent.RECRUITMENTPERIOD, 
				"开启募集期奖励，并设置每天的利率", error);
		/*验证保存操作记录是否成功*/
		if (error.code < 0) {
			JPA.setRollbackOnly();
		...
		currentBackstageSet.recruitmentPeriod = recruitmentPeriod;
		currentBackstageSet.interest = interest;
...
//		backstageSet.vipTimeLength = Integer.parseInt(options.get(62)._value);//VIP服务时长(单位[年,月]由vipTimeType确定)
		...
		backstageSet.recruitmentPeriod = options.get(109)._value;//募集期奖励功能
		backstageSet.interest = options.get(110)._value;//每天利率
		

		backstageSet.smsAppid = options.get(111)._value; // 短信通道AppID
		backstageSet.smsServer = options.get(112)._value; // 短信通道服务器地址
		backstageSet.smsServPort = options.get(113)._value; // 短信通道服务器端口
		backstageSet.smsTemplateId = options.get(114)._value; // 短信通道短信模板ID
		backstageSet.smsTimeLimit = options.get(115)._value; // 短信验证码有效期
		backstageSet.smsLimit = Long.valueOf(options.get(115)._value) * 60000L; // 短信验证码有效期
		
		StatHomePage.doStat();
		backstageSet.regPeople = Cache.get(HOME_PG_REG_PEOPLE) + "";
		backstageSet.investPeople = Cache.get(HOME_PG_INV_PEOPLE) + "";
		backstageSet.loanMoney = Cache.get(HOME_PG_LOAN_MONEY) + "";
		backstageSet.dealMoney = Cache.get(HOME_PG_DEAL_MONEY) + "";
