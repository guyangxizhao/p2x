...
public class UserAuditItem  implements Serializable{
	private static final long serialVersionUID = 3005954220308751849L;
	public long id;
	private long _id;
	public boolean lazy;
	public String no; // 编号
	...
	public static List<UserAuditItem> queryUserAllAuditItem(long userId, String mark){
		...
		String hql = "select new t_user_audit_items(audit_item_id,is_visible, mark) from t_user_audit_items where user_id = ? group by audit_item_id";
		...
		for (ProductAuditItem pitem : pitems) {
			...
			for (t_user_audit_items tuItem : tuItems) {
				if(tuItem.audit_item_id == pitem.auditItemId){
					item.lazy = true;
					item.userId = userId;
					item.isVisible=tuItem.is_visible;
	...
	/**
	 * 我的账户-借款管理-借款申请查询-提交资料审核列表
	 * @param pageBean 分页对象
	 * @param error 信息值
	 */
	public static List<v_product_audit_items> queryProductAuditItem(int productId,int ptype,ErrorInfo error){
		error.clear();
		StringBuffer sql = new StringBuffer("");
		sql.append(SQLTempletes.V_PRODUCT_AUDIT_MASTERIALS_IPS);
		List<v_product_audit_items>  v=v_product_audit_items.find(" product_id = ? and ptype=?",productId,ptype).fetch();
		 return v;
	}
	/**
	 * 借款资料审核认证
	...
	public static PageBean<v_user_audit_items> queryUserAuditItem(String currPage, String pageSize, long userId, ErrorInfo error, String ... str){
		...
		if(StringUtils.isNotBlank(str[3])){
			...
			if (productId != Constants.SEARCH_ALL) {
				conditions.append(" and audit_item_id in (select audit_item_id from t_product_audit_items where product_id = ?");
		...
		} else if (StringUtils.isNotBlank(str[4])) {
			conditions.append(" and audit_item_id ");
			if ("0".equals(str[4])) {
				conditions.append("not ");
			}
			conditions.append("in (select audit_item_id from t_product_audit_items where type=1)");
		}
	...
	public static List<AuditItem> queryAuditItemsOfOverBorrow(long userId, ErrorInfo error) {
		...
		try {
			tItems = t_dict_audit_items.find(sqlCondition, userId, Constants.AUDITING, Constants.AUDITED, Constants.UPLOAD).fetch();
		}catch(Exception e) {
			Logger.error("查询超额借款可提交的审核资料时："+e.getMessage());
			error.code = -1;
			error.msg = "数据库异常";
	...
	public void audit(long supervisorId, int auditStatus, boolean isVisible, String suggestion, ErrorInfo error){
		...
		case Constants.NOT_PASS:
			...
			if (Constants.IPS_ENABLE) {
				/* 完全资金托管模式  */
				if (Constants.PAY_TYPE_ITEM_REFUND == PayType.IPS) {
	...
	public static void submitUploadedItems(long userId, double balance, ErrorInfo error) {
		...
		String sql = "update t_user_audit_items set status = ?, submit_time = ? where status = ? and user_id = ?";
