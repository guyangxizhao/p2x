...
public class FinanceSettingAction extends SupervisorController {
	 ...
	 * 募集期利息计算设置
	 */
	public	static void settingRaiseInterest(String recruitmentPeriod,String interest){
		ErrorInfo error = new ErrorInfo();
	
		try {
			/*强转判断*/
			double double_interest	=	Double.valueOf(interest);
		} catch (NumberFormatException e) {
			e.printStackTrace();
			Logger.info("利率强转失败："+e.getMessage());
			error.code = -1;
			error.msg = "利率强转失败！";		
			renderJSON(error);
		}
		...
		backstageSet.recruitmentPeriod = recruitmentPeriod;
		backstageSet.interest = interest;
	
		backstageSet.settingRaiseInterest(error);
	 ...
	 * 募集期利息计算设置
	 */
	public	static	void	raiseInterest(){
		BackstageSet backstageSet = BackstageSet.getCurrentBackstageSet();
		System.out.println("-----"+backstageSet.recruitmentPeriod+"--+---");
		System.out.println("-----"+backstageSet.interest+"--+---");
		render(backstageSet);
	}
