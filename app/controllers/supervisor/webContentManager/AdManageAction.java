...
//	}
	...
	public static void increasedPartner(String addPartner, String imageFileName, String addUrl, int addOrder, String addDescription,
		...
		if (StringUtils.isBlank(imageFileName)||imageFileName.trim().equals("/public/images/default.png")) {
			flash.error("请上传图片");
			partner();
		}
		
		if (StringUtils.isBlank(imageType)) {
			flash.error("图片格式不符合要求"+imageFileName);
			partner();
		}
		
		if (StringUtils.isBlank(addPartner)) {
			flash.error("名称不能为空");
			partner();
		}
		
		if (StringUtils.isBlank(addDescription)) {
			flash.error("描述不能为空");
			partner();
		}
