...
public class InvestAccount extends BaseController {
	 ...
	 * 我的理财账单
	 */
	public static void investBills(int payType, int isOverType, int keyType, String key, int currPage,String type){
		if(type==null){
			type="0";
		}
		ErrorInfo error = new ErrorInfo();
        User user = User.currUser();
		long userId= user.id;
		PageBean<v_bill_invest> page = BillInvests.queryMyInvestBills(payType, isOverType, keyType, key, currPage, userId, error,type);
	 ...
	 * 我的理财账单异步
	 */
	 public static void investBillsTable1(int payType, int isOverType, int keyType, String key, int currPage,String type){
		if(type==null){
			type="0";
		}
		ErrorInfo error = new ErrorInfo();
	 ...
	 public static void investBillsTable2(int payType, int isOverType, int keyType, String key, int currPage,String type,int pageSize){
			if(type==null){
				type="0";
			}
			ErrorInfo error = new ErrorInfo();
	        User user = User.currUser();
			long userId= user.id;
			PageBean<t_bids> page = BillInvests.queryPageById2(userId, currPage, pageSize, error);
		
			render(page,type);
		}
	 public static void investBillsTable3(int payType, int isOverType, int keyType, String key, int currPage,String type){
			if(type==null){
				type="0";
			}
			ErrorInfo error = new ErrorInfo();
	...
	public static void investBillDetails(String investId,int currPage){
	    ...
	    v_bill_invest_detail investDetail=null;
		long id =0;
		if(bidId==null||bidId.equals("")){
		 id = Security.checkSign(investId, Constants.BILL_ID_SIGN, 3600, error);
		 investDetail = BillInvests.queryMyInvestBillDetails(id, user.id, error);
		}else{
		 id=Long.parseLong(bidId);
		 investDetail=new v_bill_invest_detail();
		 investDetail.bid_id=id;
		}
		
		//Bid bid = Bid.queryBidForDetails(investDetail.bid_id, error);
		Bid bid= new Bid();
		bid.id=investDetail.bid_id;
        /* 进入详情页面增加浏览次数 */
		Invest.updateReadCount(investDetail.bid_id, error);
		if (error.code < 0) {
			render(Constants.ERROR_PAGE_PATH_FRONT);
		}
...
//			Cache.add(pMerBillNo, debtId, IPSConstants.CACHE_TIME);
	...
	public static void saveOrUpdateRobot(int validType, int validDate, double minAmount, double maxAmount){
		...
		if (Constants.IPS_ENABLE && error.code >= 0) {
			...
			if("2.0".equals(BackstageSet.getCurrentBackstageSet().entrustVersion) && StringUtils.isNotBlank(authNo)){
				flag = false;
			}
...
public static void guarantee(String sign){
	 ...
	 * 	
	 */
	public static void investBillsVew(String sign,int currPage,int pageSize,int isOverType,int keyType,String keyStr,int currPageStr,int payType){
        ErrorInfo error =new ErrorInfo();
       
		long bidId=Security.checkSign(sign, Constants.BILL_ID_SIGN, 3600, error);
		User user=User.currUser();
		t_bill_invests billInvest=null;
		billInvest=BillInvests.queryInvestsByBillId(bidId);
		PageBean<t_bill_invests> page=BillInvests.queryPageById(billInvest.bid_id, user.id, currPage, pageSize, error);
		 UserOZ account = null;
			if (user != null)
				account = new UserOZ(user.id);
		render(billInvest,page,sign,account);
	}	
}